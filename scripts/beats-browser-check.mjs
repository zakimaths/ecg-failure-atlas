...
try{for(const browser of browsers){const session='atlas-beats-'+browser+'-'+process.pid;try{
 ...
 await page.setViewportSize({width:390,height:844});await page.waitForFunction(()=>{const e=document.getElementById('beat-chart');return Math.abs(e.querySelector('.main-svg').getBoundingClientRect().width-e.clientWidth)<2;});assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1),'Mobile overflow');await page.locator('#beat-chart').screenshot({path:'${output}/${browser}-beats-mobile.png'});
